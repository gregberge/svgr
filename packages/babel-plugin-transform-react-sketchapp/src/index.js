...
const plugin = ({ types: t }, { expo }) => {
  function replaceElement(path, state) {
    ...
    // Replace element by react-sketchapp components
  ...
  const importDeclarationVisitor = {
    ImportDeclaration(path, state) {
      if (path.get('source').isStringLiteral({ value: 'react-sketchapp' })) {
      ...
      if (state.unsupportedComponents.size && !path.has('trailingComments')) {
        ...
        path.addComment(
          'trailing',
          ` SVGR has dropped some elements not supported by react-sketchapp: ${componentList} `,
        )
      }
    },
  }
