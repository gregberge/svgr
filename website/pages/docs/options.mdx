...
Setting this to `true` will rewrite SVG `id` attributes and their internal references so each rendered component instance gets unique IDs generated with React `useId`.

This is useful for SVGs that contain reusable definitions such as masks, clip paths, filters, gradients, or patterns and are rendered multiple times in the same DOM. It is disabled by default because some applications rely on predictable SVG IDs for CSS or scripts.

For custom JSX runtimes, set `runtimeIds: { importSource: 'custom-runtime/hooks' }` to import `useId` from a different module. The `classic-preact` JSX runtime uses `preact/hooks` automatically.

| Default | CLI Override    | API Override                                       |
| ------- | --------------- | -------------------------------------------------- |
| `false` | `--runtime-ids` | `runtimeIds: boolean \| { importSource?: string }` |
